...
EliteForce engine should add the pak90.pk3 to their baseEF directories.
If this README is from an archive (zip or bzip2) or an automated installer
it should be included, too. You can also find the file here:
...
will find a new group in your start menu called "Elite Force Holomatch 1.35"
With shortcuts to run the game using the new patch and shortcuts to this
read me file.

After installing, a new file named iostvoyHM-1.35.exe has been created. If
...
The OpenAL driver included is based on svn revision 1262 that you can get
...
support by setting the "s_useOpenAL" cvar to 0 in the console or your
config.
Previous versions (1.34 and the beta series) required you to install an
OpenALwEAX.exe that installed OpenAL drivers directly in your system. Maybe
the linker is loading these drivers instead of the newer ones distributed
with the ZIP file. If you experience problems in the sound you can try to
uninstall these system openal drivers again. You can also try removing the
DLLs directly from the WINDOWS/system directory. (back them up though!)
...
http://thilo.kickchat.com/efport-progress/bin/linux/io_eliteforce-1.35.run
Run it in a shell like:
sh iostvoyHM-1.35.run
...
release in there and don't forget the new pak90.pk3!
...
http://thilo.kickchat.com/efport-progress/patches/stvoyVM-bugfixes-1.35.diff
